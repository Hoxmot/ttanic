...
- **Init**: creates `.ttanic/config.toml` containing an explanatory header plus only the keys the wizard/flags explicitly set -- no commented schema or defaults (resolved: in a layered config, values baked into a generated file either go stale or, once uncommented, silently pin what the user never chose; same call as nvim/ghostty/kitty/yazi; key discovery is `ttanic config`'s job, M3.13) -- and an empty `.ttanic/ignore`; the manifest is created separately, at store-open time (M1.11), not by `project.Init`. The TUI wizard (`huh`) asks: compression level, ignore patterns, confirm defaults; `ttanic init` takes the same as flags with sane defaults.
...
Because the generated config.toml carries no schema (see `internal/project` Init), `ttanic config` is the discoverability surface for configuration: its help lists the keys and their allowed values, and `get` reports the effective value and which layer (default, global, project, flag) it came from.
...
- **M4 -- release.** goreleaser (darwin/linux, amd64/arm64), `go install` verified, README, generated man pages/CLI reference (`cobra/doc`), CI release pipeline. (User docs are deliberately late: CLI help, the `?` view, and godoc are written inside each M1-M3 task, so M4 documents what actually shipped.) Post-MVP backlog opens: Homebrew tap, JSONL backend, themes-from-file, single-file extraction, job queue, symlink `follow` mode, global project registry (a small SQLite db in the XDG *data* dir -- not config -- listing every ttanic project on the machine: "where are my projects", cross-project search later).
